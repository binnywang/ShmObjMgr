...
int ShmPool::Init(void * mem,size_t max_size,bool fresh) {
	if (mem == NULL || max_size < TotalSize()) {
		LOG_ERROR("invalid param|mem[%p], max_size[%zu]\n", mem, max_size);
		return -1;
	}
	...
	if (fresh) {
		...
		if (shm_pool_header_->max_size != max_size) {
			LOG_ERROR("ShmPoolHeader MISMATCH|max_size [%zu, %zu] mismatch\n", shm_pool_header_->max_size, max_size);
			return -1;
		}
		...
		for (int i = 0; i < shm_pool_header_->group_info.group_num; i++) {
			...
			if (obj_group->class_id_num != shm_pool_header_->group_info.group[i].class_id_num) {
				LOG_ERROR("ObjGroup[%d] MISMATCH|class_id_num[%d, %d] mismatch\n", i, obj_group->class_id_num, shm_pool_header_->group_info.group[i].class_id_num);
				return -1;
			}

			for (int j = 0; j < obj_group->class_id_num; j++) {
				if (obj_group->class_id[j] != shm_pool_header_->group_info.group[i].class_id[j]) {
					LOG_ERROR("ObjGroup[%d] MISMATCH|class_id[%d] [%d, %d] mismatch\n", i, j, obj_group->class_id[j], shm_pool_header_->group_info.group[i].class_id[j]);
					return -1;
				}
			}

			if (obj_group->group_id != shm_pool_header_->group_info.group[i].group_id) {
				LOG_ERROR("ObjGroup[%d] MISMATCH|group_id[%d, %d] mismatch\n", i, obj_group->group_id, shm_pool_header_->group_info.group[i].group_id);
				return -1;
			}

			if (obj_group->max_class_size != shm_pool_header_->group_info.group[i].max_class_size) {
				LOG_ERROR("ObjGroup[%d] MISMATCH|max_class_size[%zu, %zu] mismatch\n", i, obj_group->max_class_size, shm_pool_header_->group_info.group[i].max_class_size);
				return -1;
			}

			if (obj_group->max_obj_count != shm_pool_header_->group_info.group[i].max_obj_count) {
				LOG_ERROR("ObjGroup[%d] MISMATCH|max_obj_count[%u, %u] mismatch\n", i, obj_group->max_obj_count, shm_pool_header_->group_info.group[i].max_obj_count);
				return -1;
			}
			...
			if (obj_group->group_size != group_size) {
				LOG_ERROR("ObjGroup[%d] MISMATCH|group_size[%zu, %zu] mismatch\n", i, obj_group->group_size, group_size);
				return -1;
			}
			...
			if (obj_group->used_obj_count != used_obj_count) {
				LOG_ERROR("ObjGroup[%d] MISMATCH|used_obj_count[%d, %d] mismatch\n", i, obj_group->used_obj_count, used_obj_count);
				return -1;
			}
			...
			if (obj_group->used_obj_count + free_obj_count != obj_group->max_obj_count) {
				LOG_ERROR("ObjGroup[%d] MISMATCH|max_obj_count[%d] used[%d] free[%d] mismatch\n", i, obj_group->max_obj_count, obj_group->used_obj_count, free_obj_count);
				return -1;
			}
...
void* ShmPool::NewObject(ClassId class_id, ObjId & obj_id) {
	...
	if (obj_group == NULL) {
		LOG_ERROR("%s:cannot find obj_group for class_id[%d]\n", __FUNCTION__, class_id);
		return NULL;
	}
...
void ShmPool::FreeObject(const ObjId & obj_id) {
	ObjGroup* obj_group = FindGroupByGroupId(obj_id.group_id);
	if (obj_group == NULL) {
		LOG_ERROR("%s:cannot find obj_group for group_id[%d]\n", __FUNCTION__, obj_id.group_id);
		return;
	}
...
void* ShmPool::NewObject(ObjGroup* obj_group, ClassId class_id, ObjId & obj_id) {
	if (obj_group->free_index_head == -1 || obj_group->used_obj_count >= obj_group->max_obj_count) {
		LOG_ERROR("%s:obj_group used up| group_id[%d] free_index_head[%d] max_obj_count[%d] used_obj_count[%d]\n",
			__FUNCTION__, obj_group->group_id, obj_group->free_index_head, obj_group->max_obj_count, obj_group->used_obj_count);
		return NULL;
	}
...
void* ShmPool::GetObject(ObjGroup* obj_group, const ObjId & obj_id) {
	...
	*/
	if (obj_group->obj_index_[obj_id.index].obj_id.id != obj_id.id) {
		LOG_ERROR("%s:get obj error|obj_group[%d] obj_id[%zu, %zu] mismatch", __FUNCTION__, obj_group->group_id, obj_group->obj_index_[obj_id.index].obj_id.id, obj_id.id);
		return NULL;
	}
...
void ShmPool::FreeObject(ObjGroup* obj_group, const ObjId & obj_id) {
	if (obj_group->used_obj_count <= 0 || obj_group->used_index_head == -1) {
		LOG_ERROR("%s:obj_group not used|group_id[%d] used_obj_count[%d] used_index_head[%d]",
			__FUNCTION__, obj_group->group_id, obj_group->used_obj_count, obj_group->used_index_head);
		return;
	}
